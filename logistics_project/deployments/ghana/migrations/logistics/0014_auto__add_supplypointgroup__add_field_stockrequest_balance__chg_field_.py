...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding index on 'Product', fields ['sms_code']
        db.create_index('logistics_product', ['sms_code'])

        # Adding index on 'Product', fields ['product_code']
        db.create_index('logistics_product', ['product_code'])

        # Adding index on 'Product', fields ['name']
        db.create_index('logistics_product', ['name'])

        # Adding index on 'StockRequest', fields ['status']
        db.create_index('logistics_stockrequest', ['status'])

        # Adding index on 'ProductReport', fields ['report_date']
        db.create_index('logistics_productreport', ['report_date'])

        # Adding index on 'SupplyPoint', fields ['code']
        db.create_index('logistics_supplypoint', ['code'])

        # Adding index on 'SupplyPoint', fields ['name']
        db.create_index('logistics_supplypoint', ['name'])

        # Adding index on 'ProductStock', fields ['is_active']
        db.create_index('logistics_productstock', ['is_active'])

        # Changing field 'ProductStock.last_modified'
        db.alter_column('logistics_productstock', 'last_modified', self.gf('django.db.models.fields.DateTimeField')())


    def backwards(self, orm):
        
        # Removing index on 'ProductStock', fields ['is_active']
        db.delete_index('logistics_productstock', ['is_active'])

        # Removing index on 'SupplyPoint', fields ['name']
        db.delete_index('logistics_supplypoint', ['name'])

        # Removing index on 'SupplyPoint', fields ['code']
        db.delete_index('logistics_supplypoint', ['code'])

        # Removing index on 'ProductReport', fields ['report_date']
        db.delete_index('logistics_productreport', ['report_date'])

        # Removing index on 'StockRequest', fields ['status']
        db.delete_index('logistics_stockrequest', ['status'])

        # Removing index on 'Product', fields ['name']
        db.delete_index('logistics_product', ['name'])

        # Removing index on 'Product', fields ['product_code']
        db.delete_index('logistics_product', ['product_code'])

        # Removing index on 'Product', fields ['sms_code']
        db.delete_index('logistics_product', ['sms_code'])

        # Changing field 'ProductStock.last_modified'
        db.alter_column('logistics_productstock', 'last_modified', self.gf('django.db.models.fields.DateTimeField')(auto_now=True))
