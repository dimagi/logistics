    ...
    def handle(self, text):
        ...
        if command in ["la"]:
            for c in sp_target.active_contact_set:
                send_if_connection(c, _(config.Messages.LOSS_ADJUST_HELP))
        ...
        if command in  ["soh", "hmk"]:
            for c in sp_target.active_contact_set:
                send_if_connection(c, _(config.Messages.SOH_HELP_MESSAGE))
            SupplyPointStatus.objects.create(supply_point=sp_target,
        ...
        if command in  ["fw"]:
            for c in sp_target.active_contact_set:
                send_if_connection(c, fw_message)
            self.respond(_(config.Messages.TEST_HANDLER_CONFIRM))
        if command in  ["supervision"]:
            for c in sp_target.active_contact_set:
                send_if_connection(c, _(config.Messages.SUPERVISION_REMINDER))
            SupplyPointStatus.objects.create(supply_point=sp_target,
        ...
        if command in  ["si"]:
            ...
            if sp_target.type.code.lower() == config.SupplyPointCodes.FACILITY:
                for c in sp_target.active_contact_set:
                    send_if_connection(c, _(config.Messages.STOCK_INQUIRY_MESSAGE), **{"product_name":product.name,
                                                                                        "msd_code":product.product_code})
                self.respond(_(config.Messages.TEST_HANDLER_CONFIRM))
            else:
                # this looks wrong...
                self.respond_error(_("Can only initiate product inquiry for a single facility via SMS - %(location_name)s is a %(location_type)s"),
                                    **{"location_name":"changeme",
                                       "location_type":"changeme"})
        if command in ["randr"]:
            if sp_target.type.code.lower() == config.SupplyPointCodes.DISTRICT:
                for c in sp_target.active_contact_set:
                    send_if_connection(c, _(config.Messages.SUBMITTED_REMINDER_DISTRICT))
                SupplyPointStatus.objects.create(supply_point=sp_target,
                ...
                for c in sp_target.active_contact_set:
                    send_if_connection(c, _(config.Messages.SUBMITTED_REMINDER_FACILITY))
                SupplyPointStatus.objects.create(supply_point=sp_target,
        ...
        if command in ["delivery"]:
            if sp_target.type.code.lower() == config.SupplyPointCodes.DISTRICT:
                for c in sp_target.active_contact_set:
                    send_if_connection(c, _(config.Messages.DELIVERY_REMINDER_DISTRICT))
                SupplyPointStatus.objects.create(supply_point=sp_target,
                ...
                for c in sp_target.active_contact_set:
                    send_if_connection(c, _(config.Messages.DELIVERY_REMINDER_FACILITY))
                SupplyPointStatus.objects.create(supply_point=sp_target,
        ...
        if command in ["latedelivery"]:
            ...
            for c in sp_target.active_contact_set:
                send_if_connection(c, _(config.Messages.DELIVERY_LATE_DISTRICT),
                                    **{"group_name":"changeme",
                                       "group_total":1,
        ...
        if command in ["randr_report"]:
            ...
            for c in sp_target.active_contact_set:
                send_if_connection(c, config.Messages.REMINDER_MONTHLY_RANDR_SUMMARY, **reports.construct_randr_summary(sp_target))
            self.respond_success()
        if command in ["soh_report"]:
            assert(sp_target.type.code.lower() == config.SupplyPointCodes.DISTRICT)
            for c in sp_target.active_contact_set:
                send_if_connection(c, config.Messages.REMINDER_MONTHLY_SOH_SUMMARY, **reports.construct_soh_summary(sp_target))
            self.respond_success()
        if command in ["delivery_report"]:
            assert(sp_target.type.code.lower() == config.SupplyPointCodes.DISTRICT)
            for c in sp_target.active_contact_set:
                send_if_connection(c, config.Messages.REMINDER_MONTHLY_DELIVERY_SUMMARY, **reports.construct_delivery_summary(sp_target))
            self.respond_success()
        if command in ["soh_thank_you"]:
            ...
            for c in sp_target.active_contact_set:
                send_if_connection(c, _(config.Messages.SOH_THANK_YOU))
            self.respond_success()
