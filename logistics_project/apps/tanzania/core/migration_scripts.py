...
def populate_historical_data():
	...
	# TODO: FIX, this isn't right...need to record "on time", "late", etc
	for sps in SupplyPointStatus.objects.all():
		...
		if sps.status_type=='del_fac':
			DeliveryResponse(organization = org, date = status_date, response = status_value)
		if sps.status_type=='del_dist':
			DeliveryResponse(organization = org, date = status_date, response = status_value)
		elif sps.status_type=='soh_fac':
			SOHSubmission(organization = org, date = status_date, response = status_value)
		elif sps.status_type=='rr_fac':
			RRSubmission(organization = org, date = status_date, response = status_value)
		if sps.status_type=='rr_dist':
			DeliveryResponse(organization = org, date = status_date, response = status_value)
		elif sps.status_type=='super_fac':
			SupervisionSubmission(organization = org, date = status_date, response = status_value)
