...
class Organization(models.Model):
    ...
    def save(self, *args, **kwargs):
        def new_ancestor(org):
            obl = OrganizationBloodLine(child_org=org, ancestor_org=org.parent_organization)
            obl.save()
            if org.parent_organization.parent_organization:
                new_ancestor(org.parent_organization)
        new_ancestor(self)
        super(Organization,self).save(*args,**kwargs)

class OrganizationBloodLine(models.Model):
    """
    Flattened Organization parent-child hierarchy
    """
    child_org = models.ForeignKey('Organization', related_name="child")
    ancestor_org = models.ForeignKey('Organization', related_name="ancestor")
    create_date = models.DateTimeField(auto_now_add=True)
    update_date = models.DateTimeField(auto_now=True)

    class Meta:
        unique_together = ('child_org', 'ancestor_org')
...
class SubmissionResponseStatus(object):
    ...
    CHOICES = [ON_TIME, LATE, NOT_SUBMITTED, NO_RESPONSE, REMINDER_SENT]
...
class DeliveryResponseStatus(object):
    ...
    NO_RESPONSE = "no response"
    REMINDER_SENT = "reminder sent"
    CHOICES = [RECEIVED, NOT_RECEIVED, NO_RESPONSE, REMINDER_SENT]
    RESPONSE_CHOICES = ((val, val) for val in CHOICES)
